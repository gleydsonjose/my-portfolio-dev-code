...
const NavbarTopLanguageSelect: FC<NavbarTopLanguageSelectProps> = ({
  ...
  const [selectCurrentOption, setSelectCurrentOption] =
    useState<NavbarTopLanguageSelectLanguages>({...currentLanguageData, status: true});
  ...
  function handleLanguageOption(id: number, imgSrc: string, imgAlt: string, text: string, languageTag: string) {
    dispatchIndexGroup({type: 'current-language', newStateValue: languageTag})
    setSelectCurrentOption({id, imgSrc, imgAlt, text, languageTag, status: true});
    setSelectStatus(false);
  }
        ...
        <ul className="navbar-top__language-option">
          {languageSelectData.languages.map(item => 
            <li className="navbar-top__language-option-item" key={item.id}
              onClick={() => handleLanguageOption(item.id, item.imgSrc!, item.imgAlt!, item.text!, item.languageTag!)}>
